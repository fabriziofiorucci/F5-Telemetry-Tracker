...
then
	...
	then
		echo -e "Fatal Error:\n\nRunning in local mode: $LOCAL_IMAGE not found\nCheck the documentation at https://github.com/fabriziofiorucci/F5-Telemetry-Tracker/tree/production/contrib/bigiq-docker\n"
		exit -1
	fi
fi
...
case $MODE in
	'start')
		...
		if [ "$LOCAL_MODE" = "yes" ]
		then
			echo "-> Decompressing F5 Telemetry Tracker docker image"
			gzip -d $LOCAL_IMAGE.gz >/dev/null 2>/dev/null
			echo "-> Loading F5 Telemetry Tracker docker image"
