    ...
    try:
      if nc_mode == 'NGINX_CONTROLLER':
        if pushmode == 'CUSTOM':
          ...
          payload=nc.ncInstances(fqdn=nc_fqdn,username=nc_user,password=nc_pass,mode='PUSHGATEWAY',proxy=proxyDict)
...
def getInstances():
  if nc_mode == 'NGINX_CONTROLLER':
    return Response(nc.ncInstances(fqdn=nc_fqdn,username=nc_user,password=nc_pass,mode='JSON',proxy=proxyDict),mimetype='application/json')
  elif nc_mode == 'NGINX_INSTANCE_MANAGER':
